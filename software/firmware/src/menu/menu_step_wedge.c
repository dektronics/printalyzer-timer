...
menu_result_t menu_step_wedge_calibration(step_wedge_t *wedge)
{
    ...
    do {
        densitometer_enable(DENSITOMETER_MODE_TRANSMISSION);
...
uint8_t menu_step_wedge_densitometer_input_poll_callback(uint8_t current_pos, uint8_t event_action, void *user_data)
{
    ...
    uint16_t reading = menu_step_wedge_densitometer_data_callback(event_action, NULL);
