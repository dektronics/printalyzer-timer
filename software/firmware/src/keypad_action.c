...
osStatus_t keypad_action_wait(keypad_action_t *action, int msecs_to_wait)
{
    ...
    if (keypad_event.key == KEYPAD_USB_KEYBOARD && keypad_event.pressed) {
        const char key_char = keypad_usb_get_ascii(&keypad_event);

        keypad_key_t key_equiv;
        if (key_char == '+' || key_char == 'A') {
            key_equiv = KEYPAD_ADD_ADJUSTMENT;
        } else {
            key_equiv = keypad_usb_get_keypad_equivalent(&keypad_event);
        }
